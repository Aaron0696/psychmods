...
The below sections are split into two large chunks. The `Codes` tab would display the codes that I used to extract and wrangle the data. It is split into five phases with the last one being random graphs that I thought would be useful to take a glance at. The `Post` tab is a little write-up that attempts to find the most popular modules in NUS Psychology. 

The codes and documents that I used to generate this page can also be found in the [Github Repository](https://github.com/Aaron0696/psychmods). I can also be contacted through [LinkedIn](https://www.linkedin.com/in/aaron-lim-b30898135/) or my email at aronlimjm@gmail.com.
...
#### Modules With Different Module Codes
...
```{r, fig.width = 9, eval = gitswitch, out.height = "80%", out.width = "80%", class.source = "fold-show"}
mydata %>%
...
Credits to *Jon Spring* in the tread at https://stackoverflow.com/questions/53162821/animated-sorted-bar-chart-with-bars-overtaking-each-other

##### Honour GIF

```{r Honourgif, eval = FALSE, class.source = "fold-show"}
...
```{r labgif, eval = FALSE, class.source = "fold-show"}
# df is a temporary dataframe used to generate the gif
df <- mydata %>%
...
```{r elegif, eval = FALSE, class.source = "fold-show"}
# df is a temporary dataframe used to generate the gif
df <- mydata %>%
...
<div class = "tom">
Tom is an undergraduate in NUS Psychology and beginning his final year of studies. It is the beginning of the semester and the time for [module balloting](http://www.nus.edu.sg/ModReg/), which determines the modules that Tom will be studying in the coming semester. To graduate with a Honours Degree, Tom needs to complete 5 Honours modules. As Tom is looking to be a *Clinical Psychologist* in the future, he hopes to study modules relating to *Clinical Psychology*. Whenever Tom thinks and ponders, the text box would be highlighted in blue.
</div>
...
```{r comparecat, fig.width = 4, fig.height = 3, out.width = "25%", out.height = "20%", eval = htmlswitch, echo = gitswitch}
...
```{r lsbbytime, out.width = "40%", out.height = "40%", fig.hold = "hold", fig.width = 7, fig.width = 7, eval = htmlswitch, echo = gitswitch}
...
```{r lablsbbytime, out.width = "50%", out.height = "50%", fig.hold = "hold", fig.width = 7, fig.width = 7, eval = htmlswitch, echo = gitswitch, fig.cap = "this is a caption!", fig.align = "center"}
...
```{r cdfquota, fig.height = 3, fig.width = 3, out.width = "25%", out.height = "25%", eval = htmlswitch, echo = gitswitch}
...
```{r elelsbbytime, out.width = "50%", out.height = "50%", fig.hold = "hold", fig.width = 7, fig.width = 7, eval = htmlswitch, echo = gitswitch}
# same comments as above code in Honours section
