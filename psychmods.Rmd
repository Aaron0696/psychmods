...
## Collapsing Across All Accounts

```{r}
# aggregate across all accounts within a round of bidding
mydata <- aggregate(. ~ AcadYear + Semester + Round + ModuleCode + Level + StartTime + DayText + LessonTime,
               data = mydata[,-grep("StudentAcctType", names(mydata))],
               FUN = mean)
```
...
for(r in unique(mydata$ModuleCode))
{
  plot(
    mydata %>%
      filter(ModuleCode == r) %>%
      ggplot(aes(x = StartTime, y = BpQ, color = Round)) +
      geom_point() +
      ggtitle(r) + 
      facet_grid(~ DayText) +
      theme_bw() + 
